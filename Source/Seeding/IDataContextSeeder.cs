...
namespace Janus.Seeding
{
    ...
    {
        void SeedDataContext<TContext>(TContext context) where TContext : DbContext;
    }

    public class DataContextSeeder : IDataContextSeeder, ISeedReader
    {
        private readonly ISeederFactory seederFactory;
        private readonly ILogger<DataContextSeeder> logger;

        public DataContextSeeder(ISeederFactory seederFactory, ILogger<DataContextSeeder> logger)
        {
            this.seederFactory = seederFactory ?? throw new System.ArgumentNullException(nameof(seederFactory));
        ...
        public void SeedDataContext<TContext>(TContext context) where TContext : DbContext
        {
            IEntitySeeder[] seeders = this.seederFactory.CreateSeeders();
        ...
        private Dictionary<Type, object[]> GetAllSeedData(IEntitySeeder[] seeders)
        {
            ...
            {
                throw new DuplicateSeederException($"Duplicate seeders have been used for seeding data. The {nameof(ISeederFactory)} must provide distinct seeders. See inner exception for details on which seeder is a duplicate.", ex);
            }
        }
    }
}
